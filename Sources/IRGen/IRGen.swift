...
public func irgen(statements: [Parser.Statement]) -> [Codegen.Statement] {
    ...
    for statement in statements {
        ...
        case .variableDeclaration(let variable, initializer: let initializer):
            result.append(.variable(variable.name, type: variable.type.description, initializer: initializer.map(irgen(expression:)) ?? .literal("NULL")))
        ...
        case .functionDeclaration(let name, returns: let returnType, parameters: let parameters, body: let body):
            result.append(.function(name, returns: returnType?.description ?? "void", parameters: [], body: irgen(statements: body)))
